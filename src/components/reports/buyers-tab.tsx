...
export function BuyersTab({ baseParams, onDrillDown }: BuyersTabProps) {
  ...
  const { data, isPending, isError, refetch } = useBuyersReport(params);
  ...
  const top10 = useMemo(() => {
    ...
    return [...data.items]
      ...
      .slice(0, 10)
      .map((b) => ({
        name: b.buyer_name || b.buyer_gstin,
        amount: b.total_amount,
      }));
  }, [data]);

  if (isError) {
    return <ErrorState title="Failed to load buyers report" onRetry={() => refetch()} />;
  }

  return (
    <div className="space-y-6">
      <ChartCard
        title="Top 10 Buyers by Amount"
        loading={isPending}
